...
## Backlog
Use `bd` for active planning and execution. Keep `IDEAS.md` as legacy ideation notes only.

## Issue Tracking
Use `bd` (Beads) for all task tracking.
Do not use markdown TODO files or ad-hoc task lists.

## Standard Workflow
Before starting any implementation work:
1. Run `bd ready` and inspect the most relevant ready issue.
2. Read the issue details with `bd show <id>`.
3. Implement only what belongs to that issue.
...
- Prefer `bd --sandbox <command>` for normal issue operations (`ready`, `show`, `create`, `update`, `close`, `status`) to avoid local socket access entirely.
- If `bd doctor --server` or other server-mode commands report `connect: operation not permitted`, re-run with elevated/unrestricted execution in the agent.
